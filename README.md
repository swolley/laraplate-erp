...
The reconciled implementation baseline is documented in [docs/STATUS.md](docs/STATUS.md). All approved mandatory ERP work that does not require external `/api/v1` exposure is implemented. External-source importers are tracked separately and are not part of this baseline.
...
-   Core Settings hides historical `version_strategy_erp_*` rows for those class-forced DIFF models; the rows are retained but cannot mislead administrators
...
-   Locked order lines reject commercial changes (`qty_ordered`, item, description, source and price) and deletion, while operational counters/status remain writable
-   SQLite and Oracle use the equivalent Eloquent guards; database triggers remain defense in depth where installed
...
-   Revision history is linear: revise the latest quotation; a source with an existing successor cannot branch. Editable unlocked drafts are changed directly rather than revised.
-   Filament exposes **Create revision** for locked or non-draft quotations and redirects to the new draft.
-   A project linked to a confirmed/partially/fully evaded sales order is locked automatically. Model validation blocks update/delete and Filament hides edit/delete access.
-   DDT linkage locks the source sales-order line. Commercial values become immutable, while delivery, invoice and return counters and operational status can still progress.
-   Application locking is portable; MySQL/MariaDB and PostgreSQL add equivalent database trigger defense against raw SQL bypasses.
...
-   **Resources:** Party, Contact, Site, Quotation, Project, Lead, Opportunity, SalesOrder, DeliveryNote, Invoice, PurchaseOrder, GoodsReceipt, ReturnOrder, SupplierReturn, PaymentTerm, Payment, PaymentRun, PaymentRequest, PartnerPool, BankAccount, BankStatement, VatRegister (read-only), VatSettlement (read-only)
-   ERP `Site` records select a canonical Core `Place`; address, city, province, postcode and country are not duplicated in ERP. Site validity uses the standard Core `HasValidity` contract.
...
| Phase 4 commercial depth | Required scope implemented | Cash sharing, Payment Requests, canonical Site/Place locations, Task UI, and RFC 5545 ICS export are present. Gantt (`4-08`) and mobile API (`4-13`) are optional and unapproved. |
...
### Point 0 Go-Live and Future Boundaries
...
-   Registered domain HTTP actions are available on the authenticated internal `/app` surface. External `/api/v1` actions and ERP-specific resources remain deferred until a concrete consumer and governance contract are approved.
...
-   MES remains a separate vertical. External-source importers are a separate workstream. Gantt and mobile API are optional; calendar/ICS export and partner-pool settlement are implemented. External money transfer remains separate.

### Roadmap

-   ERP Point 0: approved mandatory non-external-API scope is implemented; see [docs/STATUS.md](docs/STATUS.md).
-   A new implementation wave starts only after explicitly approving external API governance, optional Gantt/mobile scope, or another independent requirement.
