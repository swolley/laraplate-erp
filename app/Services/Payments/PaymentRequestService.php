...
final readonly class PaymentRequestService
{
    ...
    {
        return DB::transaction(function () use ($request): PaymentRequest {
    ...
    {
        return DB::transaction(function () use ($provider, $payload): PaymentRequest {
            ...
            $request = PaymentRequest::query()->where('provider_code', $provider)
                ->where('external_id', $external_id)->lockForUpdate()->firstOrFail();
